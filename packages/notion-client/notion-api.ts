...
export class NotionAPI {
  ...
  public async getPage(
    ...
    // ✅ [안전한 수정] 작성자 정보를 가져오다가 에러가 나도 본문 로딩은 계속되게 합니다.
    try {
      ...
      console.warn('⚠️ 작성자 정보를 가져오지 못했지만 본문 데이터를 우선 처리합니다.');
    }
...
public async getUsers(userIds: string[], gotOptions?: OptionsOfJSONResponseBody) {
  ...
  try {
    ...
    >({
      ...
      body: {
        requests: validUserIds.map(id => ({ id, table: 'notion_user' })),
      },
      gotOptions,
    });
  } catch (err) {
    // 💡 에러 발생 시 로그만 남기고 빈 배열 반환하여 다음 로직 진행 허용
    console.warn('❌ Notion API getUsers failed (400 error ignored):', err.message);
  ...
  // packages/notion-client/notion-api.ts 맨 아래 fetch 함수
  ...
  public async fetch<T>({
    ...
    // 2. 💡 [AaRC 전역 평탄화 엔진 V3] 데이터 정제 작업을 시작합니다.
    let result = response;

    if (result) {
      // [공정 A] 가장 바깥쪽 껍질(value.value)이 있다면 먼저 벗깁니다.
      if (result.value && result.value.value) {
        result = result.value.value;
      }

      // [공정 B] recordMap 내부의 데이터들(block, collection 등)을 순회하며 해체합니다.
      if (result.recordMap) {
        const categories = ['block', 'collection', 'collection_view', 'notion_user', 'space'];
        
        categories.forEach((category) => {
          const categoryData = result.recordMap[category];
          if (categoryData) {
            Object.keys(categoryData).forEach((key) => {
              const item = categoryData[key];
              // 개별 아이템이 다시 이중 포장되어 있다면 해제
      ...
      // [공정 C] 검색 결과나 작성자 목록(results)이 배열로 들어온 경우 처리합니다.
